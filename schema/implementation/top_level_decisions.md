...
| **Gate 3** | Agent Lifecycle | Agent Runtime loads soul, assembles prompt, calls LLM. Pool Controller tracks heartbeats, routes tasks. Orchestrator submits task → agent processes → output promoted. First end-to-end flow works. | ◻ Next |
| **Gate 4** | Safety & Quality | Sandbox Pipeline scans/builds/tests output. Recovery Controller replays checkpoint. Evaluation Engine detects drift. | ◻ |
