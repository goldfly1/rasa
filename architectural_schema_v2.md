...
> **Status:** Draft v2.0 — additions from strategic_review integrated; open for implementation planning  
...
| **Message Bus** | Envelope JSON | ACK / NACK + watermark | Retry 3×; then dead-letter to `ESCALATED` queue |
...
The Kill Switch is a control-plane signal emitted by the Recovery Controller and consumed by the Orchestrator. It is **not** a bus message; it uses a dedicated side-channel to ensure delivery even when the Message Bus is saturated or partitioned.
...
| `prompt_version` | string | 1 | Active instruction set at time of run. |
...
*End of Architectural Schema v1.0*
